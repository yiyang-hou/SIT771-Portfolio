...
private void AAA(){SplashKit.LoadBitmap("Bullet", "Aquarii.png");SplashKit.LoadBitmap("Gliese", "Gliese.png");SplashKit.LoadBitmap("Pegasi", "Pegasi.png");SplashKit.LoadBitmap("Aquarii", "Fire.png");}public void AAB(){_gameWindow = new Window("BlastOff", 600, 600);while (!_gameWindow.CloseRequested){SplashKit.ProcessEvents();if (SplashKit.KeyDown(KeyCode.UpKey)){a.Move(4, 0);}if (SplashKit.KeyDown(KeyCode.DownKey)){a.Move(-4, 0);}if (SplashKit.KeyDown(KeyCode.LeftKey)){a.Rotate(-4);}if (SplashKit.KeyDown(KeyCode.RightKey)){a.Rotate(4);}if (SplashKit.KeyTyped(KeyCode.SpaceKey)){a.Shoot();}a.TODORENAME();ABA();}_gameWindow.Close();_gameWindow = null;}private void ABA(){_gameWindow.Clear(Color.Black);a.Draw();_gameWindow.Refresh(60);}}public class Thingy{private double _x, _y;
...
public void Move(double amountForward, double amountStrafe){Vector2D movement = new Vector2D();Matrix2D rotation = SplashKit.RotationMatrix(_angle);
